...
export interface ComputedRule {
   ...
   *  percent: fields is [numeratorKey, denominatorKey], target = numerator/denominator × 100 (0 if denominator is 0). */
  formula: "sum" | "subtract" | "multiply" | "percent" | "hoursBetween";
...
export const MODULES: ModuleDef[] = [
  ...
  },
  {
    ...
    icon: "📊",
    ready: true,
    label: { zh: "菜品销量与毛利", en: "Dish Sales & Margin", fr: "Ventes et marge par plat" },
    pain: {
      zh: "不知道哪些菜卖得好、哪些卖不动，也不知道哪些菜其实不赚钱",
      en: "No idea which dishes sell well — or which ones quietly lose money.",
      fr: "Aucune idée des plats qui se vendent bien — ni de ceux qui perdent de l'argent.",
    },
    fields: [
      ...
      { key: "margin", label: { zh: "毛利", en: "Margin", fr: "Marge" }, type: "money", half: true, unit: true },
      { key: "costRate", label: { zh: "成本率", en: "Cost rate", fr: "Taux de coût" }, type: "number", suffix: "%", half: true, unit: true },
      { key: "marginRate", label: { zh: "毛利率", en: "Margin rate", fr: "Taux de marge" }, type: "number", suffix: "%", half: true, unit: true },
    ...
    computed: [
      { target: "margin", formula: "subtract", fields: ["price", "-cost"] },
      { target: "costRate", formula: "percent", fields: ["cost", "price"] },
      { target: "marginRate", formula: "percent", fields: ["margin", "price"] },
    ],
    outputs: [
      ...
      { zh: "填入成本价，自动算出毛利和成本率", en: "Enter a cost and margin / cost-rate compute automatically", fr: "Saisissez un coût, la marge et le taux de coût se calculent automatiquement" },
    ],
    amountKey: "revenue",
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "status", label: { zh: "状态", en: "Status", fr: "Statut" }, type: "select", half: true,
        options: [{ zh: "待处理", en: "Open", fr: "À traiter" }, { zh: "处理中", en: "In progress", fr: "En cours" }, { zh: "已完成", en: "Done", fr: "Terminé" }] },
