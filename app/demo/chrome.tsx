  ...
  return (
    <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50/80 px-4 py-2.5">
      ...
      <div className="ml-auto flex items-center gap-2">
        <span className="rounded-full bg-sky-50 px-2 py-0.5 text-[11px] font-semibold text-sky-700">{t({ zh: "演示", en: "Demo" })}</span>
        <LangToggle />
        <Link href="/" className="hidden text-xs text-slate-500 transition hover:text-slate-800 sm:inline">
          {t({ zh: "← 返回官网", en: "← Back to site" })}
        </Link>
        <Link
          href="/login"
...
export function DemoFootnote() {
  ...
  return (
    <p className="mt-3 text-center text-xs text-slate-400">
      {t({ zh: "演示用示例数据，数字仅供参考。", en: "Sample data for demonstration — numbers are illustrative." })}
    </p>
  );
}
