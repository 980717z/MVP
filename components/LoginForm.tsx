...
const T: Record<string, Dict> = {
  ...
  campusWord: { en: "Campus", zh: "校园", fr: "Campus" },
  footer: {
...
// Shared sign-in form for both the platform login (/login) and the campus login
// (/campus/login). Same auth flow + redirect; `campus` only swaps the wordmark to
// "BentoOS Campus". Keeping ONE implementation avoids duplicating the iOS-Safari
// fixes (uncontrolled inputs, storage-persistence check, no native form reload).
export default function LoginForm({ campus = false }: { campus?: boolean }) {
      ...
      <div className="w-full max-w-sm">
        <div className="mb-6 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <BentoMark className="h-9 w-9 shadow-sm" />
            {campus ? (
              <span className="text-lg tracking-tight">
                <span className="font-extrabold text-slate-900">BentoOS</span>
                <span className="ml-1 font-semibold text-brand-ink/70">{t(T.campusWord)}</span>
              </span>
            ) : (
